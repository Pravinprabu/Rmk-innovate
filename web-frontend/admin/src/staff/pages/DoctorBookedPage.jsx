...
export default function DoctorBookedPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const result = await callApi(() => doctorApi.bookings());
      setLoading(false);
  ...
  return (
    <StaffShell role="doctor" title="Booked Patients" profileName={session?.profile?.full_name}>
